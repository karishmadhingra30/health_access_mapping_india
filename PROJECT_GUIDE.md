...
2. Deploy the static site shell even when fresh data fails.
   ...
   The workflow now continues past an optional refresh failure and deploys the static site with the last committed public data. The sidebar reads `refresh_status`, `refreshed_at`, facility counts, and source warnings from `refresh_metadata.json`.
